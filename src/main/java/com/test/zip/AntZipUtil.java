...
public class AntZipUtil {
    ...
    public static boolean unZip(String zipFileName, String outputDirectory) throws Exception {
        ...
        try {
            ...
            // Log.error("unzip file zipFileName fail. filename" + zipFileName,
            // ex);
            throw ex;
        }
        finally {
    ...
    public static void compressByType(File dir, ZipOutputStream out, String basedir) throws Exception {
        if (dir.isDirectory()) {
            compressDirectory(dir, out, basedir);
        }
        else {
            compressFile(dir, out, basedir);
        }
    }

    public static void compressDirectory(File dir, ZipOutputStream out, String basedir) throws Exception {
        if (!dir.exists()) {
            return;
        }
        File[] files = dir.listFiles();
        for (int i = 0; i < files.length; i++) {
            compressByType(files[i], out, basedir + "/");
        }
    }

    public static void compressFile(File dir, ZipOutputStream out, String basedir) throws Exception {
